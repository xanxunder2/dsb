...
if (!args[0])
    return api.sendMessage("Wrong format!\nUse "+global.config.PREFIX+this.config.name+" "+this.config.usages, event.threadID, event.messageID);
	 ...
	 return request(encodeURI(`https://okay--s1-fucx-6x.repl.co/api/canvas/logo3?teks1=${name1}&teks2=${name2}`)).pipe(fs.createWriteStream(__dirname+'/cache/biden.png')).on('close',() => callback()); 
} catch (err){
