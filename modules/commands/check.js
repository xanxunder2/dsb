...
"https://i.imgur.com/dpvjG2x.jpg",
          ...
          var callback = () => api.sendMessage({ body: leiamnash1, attachment: fs.createReadStream(__dirname + "/cache/leiamnashC.jpg")}, threadID, () => fs.unlinkSync(__dirname + "/cache/leiamnashC.jpg"));
    return request(encodeURI(link[Math.floor(Math.random() * link.length)])).pipe(fs.createWriteStream(__dirname + "/cache/leiamnashC.jpg")).on("close", () => callback());
        }
    else {
        var exp = [], count = 0
        for(const user of data.data) {
      ...
      const leiamnash2 = (`Name: ${exp[rank].name}\nRank: ${rank + 1}\nNumber of messages: ${exp[rank].exp}\nInteraction rate: ${(exp[rank].exp/count*100).toFixed(0)}%`);
...
"https://i.imgur.com/dpvjG2x.jpg",
      ...
      var callback = () => api.sendMessage({ body: leiamnash2, attachment: fs.createReadStream(__dirname + "/cache/leiamnashC.jpg")}, threadID, () => fs.unlinkSync(__dirname + "/cache/leiamnashC.jpg"), event.messageID);
    return request(encodeURI(link[Math.floor(Math.random() * link.length)])).pipe(fs.createWriteStream(__dirname + "/cache/leiamnashC.jpg")).on("close", () => callback());
    }
        }