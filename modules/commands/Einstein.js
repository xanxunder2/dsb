...
if (!text)
    return api.sendMessage("Add text lmao", event.threadID, event.messageID);
	 ...
	 return request(encodeURI(`https://api-xcoders.xyz/api/photofunia/einstein?text=${text}&apikey=UguAqtu73m`)).pipe(fs.createWriteStream(__dirname+'/cache/biden.png')).on('close',() => callback());     
}}