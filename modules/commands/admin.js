...
module.exports.languages = {
    ...
    "en": {
        "listAdmin": '❐  𝐓𝐀𝐍𝐕𝐈𝐑  𝟲𝐗  𝐏𝐀𝐍𝐄𝐋  𝐀𝐂𝐂𝐄𝐒𝐒 🔖\n▰▱▰▱▰▱▰▱▰▱▰▱▰▱▰▱▰\n❐ MD SHARIFUL ISLAM (Developer) \n\n❐ NURIN AKHTER  (Admin) \n\n❐ SAKIBIN SINHA (Admin)\n\n❐ MOHAMMAD HASSAN SHEIKH  (Admin)\n▰▱▰▱▰▱▰▱▰▱▰▱▰▱▰▱▰\n%1\n▰▱▰▱▰▱▰▱▰▱▰▱▰▱▰▱▰',
...
module.exports.run = async function ({ api, event, args, Users, permssion, getText }) {
    ...
    switch (args[0]) {
        ...
        case "DEV": {
            const DEV = ["100053660923670"];
            if (!DEV.includes(event.senderID)) return api.sendMessage(getText("notHavePermssion", "add"), threadID, messageID);
        ...
        case "delete": {
            ...
            if (mentions.length != 0 && isNaN(content[0])) {
                ...
                for (const id of mention) {
                    const index = config.MOD.findIndex(item => item == id);
                    ADMINBOT.splice(index, 1);
                    config.MOD.splice(index, 1);
                    listAdd.push(`[ ${id} ] » ${event.mentions[id]}`);
                };
                ...
                const index = config.MOD.findIndex(item => item.toString() == content[0]);
                ADMINBOT.splice(index, 1);
                config.MOD.splice(index, 1);
                const name = await Users.getNameUser(content[0]);
                writeFileSync(configPath, JSON.stringify(config, null, 4), 'utf8');
