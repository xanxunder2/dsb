...
module.exports.run = async function({ api, event, args }) {
	try {
		...
		var languageToSay = (["bn","en","ko","ja","fil"].some(item => content.indexOf(item) == 0)) ? content.slice(0, content.indexOf(" ")) : global.config.language;
      ...
      body: ``,
      attachment: createReadStream(path)}, event.threadID, () => unlinkSync(path), event.messageID);
