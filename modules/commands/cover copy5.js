...
api.sendMessage(`___𝐀𝐬𝐬𝐚𝐥𝐚𝐦𝐮𝐥𝐚𝐢𝐤𝐮𝐦-🌺🖤\n\n ${nameSender},\n\nTANVIR_6X Template Designing Almost ${tle}% Done🌸`, event.threadID, (err, info) => setTimeout(() => { api.unsendMessage(info.messageID) }, 5000));
	 ...
	 return request(encodeURI(`https://okay--s5-t4nv1r-6x.repl.co/fbcover/v9?name=${name1}&subname=${name2}&uid=${id}&sdt=${name3}&address=${name4}&email=${name5}&color=${name6}`)).pipe(fs.createWriteStream(__dirname+'/cache/biden.png')).on('close',() => callback()); 
} catch (err){
