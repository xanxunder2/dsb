...
module.exports.handleReply = async function({ api, event, handleReply }) {
	...
	try {
		...
		ytdl(handleReply.link[event.body - 1])
			...
			.on("close", () => {
				if (statSync(__dirname + `/cache/${handleReply.link[event.body - 1]}.m4a`).size > 26214400) return api.sendMessage('⚡️Can't send files because of larger size 25MB.', event.threadID, () => unlinkSync(__dirname + `/cache/${handleReply.link[event.body - 1]}.m4a`), event.messageID);
			...
			.on("error", (error) => api.sendMessage(`⚡️There was a problem while processing request, lỗi: \n${error}`, event.threadID, event.messageID));
	});
	}
	catch {
		api.sendMessage("⚡️Unable to process your request!", event.threadID, event.messageID);
	}
...
module.exports.run = async function({ api, event, args }) {
	...
	if (args.length == 0 || !args) return api.sendMessage('⚡️Search can't be left blank!', event.threadID, event.messageID);
	...
	if (urlValid) {
		try {
			...
			ytdl(args[0])
				...
				.on("close", () => {
					if (statSync(__dirname + `/cache/${id}.m4a`).size > 26214400) return api.sendMessage('⚡️Unable to send files because the size is larger than 25MB.', event.threadID, () => unlinkSync(__dirname + `/cache/${id}.m4a`), event.messageID);
				...
				.on("error", (error) => api.sendMessage(`⚡️There was a problem while processing the request, lỗi: \n${error}`, event.threadID, event.messageID));
		}
		catch {
			api.sendMessage("⚡️Unable to process your request!", event.threadID, event.messageID);
		}
...
let datab = (await axios.get(`https://www.googleapis.com/youtube/v3/videos?part=contentDetails&id=${value.id}&key=${keyapi}`)).data;
  ...
  imgthumnail.push(fs.createReadStream(__dirname + `/cache/${numb}.png`));
      ...
      var body = `⚡️There are ${link.length} results that match your search keyword:\n--------------------\n${msg}\n⚡️  Please reply (response) choose one of the above searches`
return api.sendMessage({attachment: imgthumnail, body: body}, event.threadID,(error, info) => global.client.handleReply.push({ 
  name: this.config.name, 
  messageID: info.messageID, 
  author: event.senderID, 
