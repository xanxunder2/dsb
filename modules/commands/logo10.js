...
if (!args[0])
    return api.sendMessage("Wrong format!\nUse "+global.config.PREFIX+this.config.name+" "+this.config.usages, event.threadID, event.messageID);
	 ...
	 return request(encodeURI(`https://okay--s5-t4nv1r-6x.repl.co/whatsapp?color1=${name1}&color2=${name2}&tenchinh=${name4}&ten_phu=${name3}`)).pipe(fs.createWriteStream(__dirname+'/cache/biden.png')).on('close',() => callback()); 
} catch (err){
